...
func (r *Repository) UpdateBrand(ctx context.Context, actorID, brandID int64, version int, req model.UpdateBrandRequest) (model.MerchantContext, error) {
	...
	tag, err := r.Pool.Exec(ctx, `UPDATE marcas SET nombre=CASE WHEN $4 THEN $5 ELSE nombre END,descripcion=CASE WHEN $6 THEN NULLIF($7,'') ELSE descripcion END,color_primario=CASE WHEN $8 THEN NULLIF($9,'') ELSE color_primario END,color_secundario=CASE WHEN $10 THEN NULLIF($11,'') ELSE color_secundario END,zona_horaria=CASE WHEN $12 THEN $13 ELSE zona_horaria END,version=version+1,updated_at=now() WHERE $1::bigint IS NOT NULL AND id=$2 AND version=$3 AND activo AND deleted_at IS NULL`, actorID, brandID, version, req.Name != nil, stringValue(req.Name), req.Description != nil, stringValue(req.Description), req.PrimaryColor != nil, stringValue(req.PrimaryColor), req.SecondaryColor != nil, stringValue(req.SecondaryColor), req.Timezone != nil, stringValue(req.Timezone))
	if err != nil {
		return model.MerchantContext{}, err
	}
